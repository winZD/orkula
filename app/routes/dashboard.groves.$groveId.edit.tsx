...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  return { grove, existingVarieties };
}
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  await db.$transaction([
    ...
    db.groveVariety.deleteMany({ where: { groveId: grove.id } }),
    db.groveVariety.createMany({
      data: varietyEntries.map((v) => ({
        groveId: grove.id,
        variety: v.variety,
...
export default function EditGrove({ loaderData }: Route.ComponentProps) {
  const { grove, existingVarieties } = loaderData;
  ...
  const {
    ...
    defaultValues: {
      ...
      varieties: grove.varieties.map((v) => ({
        variety: v.variety,
        treeCount: v.treeCount?.toString() ?? "",
      })),
    },
  });
